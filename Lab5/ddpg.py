...
def train(args, env, agent, writer):
    ...
    for episode in range(args.episode):
        ...
        state = env.reset()
        if early_step == 15:
            model_path = f'models/ddpg/episode={str(episode)}_best.pth'
            agent.save(model_path, checkpoint=True)

            break
        if episode % 30 == 0 and episode != 0:
            model_path = f'models/ddpg/ddpg_episode={str(episode)}.pth'
            agent.save(model_path, checkpoint=True)
...
def main():
    ...
    trained_model_path = "models/ddpg/best.pth"
