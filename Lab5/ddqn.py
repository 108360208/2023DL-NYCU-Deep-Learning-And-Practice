...
class DDQN:
    def __init__(self, args):
        ...
        ## TODO ##
        self._optimizer = optim.RMSprop(self._behavior_net.parameters(), lr=args.lr, alpha=0.9)
        # memory
    ...
    def _update_behavior_network(self, gamma):
        ...
        q_value = torch.gather(input=q_value, dim=1, index=action.long())
        with torch.no_grad():
            q_next_behavior = self._behavior_net(next_state)  # Q-values from the behavior network
            _, best_action_next_behavior = q_next_behavior.max(dim=1, keepdim=True)
            q_next_target = self._target_net(next_state)  # Q-values from the target network
            q_next_target_max = q_next_target.gather(1, best_action_next_behavior)
            q_target = reward + gamma * q_next_target_max * (1 - done)
...
def train(args, env, agent, writer):
    ...
    for episode in range(args.episode):
        ...
        if episode % 100 == 0 and episode != 0:
            model_path = f'models/dqn/episode={str(episode)}.pth'
            agent.save(model_path, checkpoint=True)
...
def main():
    ...
    parser.add_argument('-m', '--model', default='models/dqn/best.pth')
    ...
    if not args.test_only:
        ...
        model_path = "models/dqn/best.pth"
